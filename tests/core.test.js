...
test('response contacts include matched bank and explain emergency numbers',()=>{const {responseContacts}=require('../src/parsers');const paid=responseContacts('paid','Tôi vừa chuyển tiền từ Vietcombank',hotlines);assert.deepEqual(paid.map(x=>x.phone),['1900545413','156','113']);assert.match(paid.find(x=>x.phone==='113').purpose,/khẩn cấp/);const safe=responseContacts('none','tin nhắn lạ',hotlines);assert.deepEqual(safe.map(x=>x.phone),['156'])});
